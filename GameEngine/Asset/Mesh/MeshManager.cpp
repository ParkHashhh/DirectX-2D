...
bool CMeshManager::Init()
{
	...
	if (!CreateMesh("Mesh_CenterRectColor",true, CenterRectColor, sizeof(FVertexColor),
	...
	if (!CreateMesh("Mesh_CenterFrameRect",true, CenterFrameRect, sizeof(FVector3),
	...
	if (!CreateMesh("Mesh_LBFrameRect",true, LBFrameRect, sizeof(FVector3),
		4, D3D11_USAGE_IMMUTABLE, D3D11_PRIMITIVE_TOPOLOGY_LINESTRIP,
	...
	if (!CreateMesh("Mesh_RectTex",true, RectTexture,
		sizeof(FVertexTex),
		4, D3D11_USAGE_IMMUTABLE, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST,
		CenterRectColorIdx, 2, 6, DXGI_FORMAT_R16_UINT,
		D3D11_USAGE_IMMUTABLE))
	...
	if (!CreateMesh("Mesh_UIRectTex",true, UIRectTexture,
		sizeof(FVertexTex),
		4, D3D11_USAGE_IMMUTABLE, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST,
		CenterRectColorIdx, 2, 6, DXGI_FORMAT_R16_UINT,
		D3D11_USAGE_IMMUTABLE))
	...
	if (!CreateMesh("Mesh_CenterCubeColor",true, CenterCubeColor, 
		sizeof(FVertexColor),
	...
	if (!CreateMesh("Mesh_LineUP2D",true, LineUp,
		sizeof(FVector3),
	...
	if (!CreateMesh("Mesh_LineRight2D",true, LineRight,
		sizeof(FVector3),
		2, D3D11_USAGE_IMMUTABLE, D3D11_PRIMITIVE_TOPOLOGY_LINESTRIP))
		return false;
...
bool CMeshManager::CreateMesh(const std::string& Name,bool Keep,
	void* VertexData, int VertexSize, int VertexCount, D3D11_USAGE VertexUsage, D3D11_PRIMITIVE_TOPOLOGY Primitive, void* IndexData, int IndexSize, int IndexCount, DXGI_FORMAT Fmt, D3D11_USAGE IndexUsage)
{
