...
void CWorldAssetManager::KeepAsset(
	const std::weak_ptr<CWorldAssetManager>& AssetMgr)
{
	auto	Mgr = AssetMgr.lock();

	auto	iter = Mgr->mAssetMap.begin();
	auto	iterEnd = Mgr->mAssetMap.end();

	for (; iter != iterEnd; ++iter)
	{
		if (mAssetMap.find(iter->first) == mAssetMap.end())
		{
			mAssetMap.insert(std::make_pair(iter->first,
			iter->second));
		}
	}
}
	...
	D3D11_USAGE IndexUsage)
{
	...
	if (MeshMgr)
	{
		...
		// 통과.
		if (!MeshMgr->CreateMesh(Key,Keep,
...
void CWorldAssetManager::SoundPlay(const std::string& Name)
{
	std::string	Key = "Sound_" + Name;
...
void CWorldAssetManager::SoundStop(const std::string& Name)
{
	std::string	Key = "Sound_" + Name;
