...
protected:
	...
	bool	mKeepAsset = false;
public:
	void EnableKeepAsset()
	{
		mKeepAsset = true;
	}
...
public:
	...
	template <typename T>
	bool FindObjectListType(std::list<std::weak_ptr<T>>& ObjList)
	{
		...
		{
			std::shared_ptr<T> CastObj = std::dynamic_pointer_cast<T>(iter->second);
			if (CastObj)
			{
				ObjList.push_back(CastObj);
			}
		}
